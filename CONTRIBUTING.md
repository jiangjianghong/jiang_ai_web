...
An effective bug report goes a long way. Use the template provided when opening an issue to ensure you provide all necessary details:
...
3.  **Create a new Branch**. We recommend using the format `feature/feature-name` or `fix/bug-name`.
...
We use [Conventional Commits](https://www.conventionalcommits.org/). This means that all commit messages should be formatted like this:
...
```
feat(auth): implement google login support
fix(ui): fix mobile padding on homepage
