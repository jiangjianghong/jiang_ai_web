        ...
        >
          ...
          <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] bg-white rounded-2xl border border-gray-100 overflow-hidden select-none">
            ...
            <div className="p-4 flex-1 flex flex-col items-center justify-center">
              ...
              {/* 标题 */}
              <h3 className="text-sm font-semibold text-gray-800 text-center mb-1 line-clamp-2 leading-tight">
                {item.title}
              </h3>

              {/* 描述 */}
              {item.description && (
                <p className="text-xs text-gray-600 text-center line-clamp-1 leading-tight opacity-70">
                  {item.description}
                </p>
              )}
            </div>
            ...
            {/* 底部状态栏 */}
            <div className="absolute bottom-0 left-0 right-0 bg-white/90 backdrop-blur-sm border-t border-gray-200/80 px-3 py-2 flex items-center justify-between">
              {/* 左下角：状态指示器 */}
              ...
              {/* 右下角：跳转按钮 */}
              <div className="cursor-pointer p-1 hover:bg-gray-100 rounded transition-colors duration-200">
                <i className="fa-solid fa-external-link-alt text-xs text-gray-600"></i>
              </div>
            </div>
          </div>

          {/* 背面 - 账号密码信息 */}
          <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-white rounded-2xl border border-gray-100 p-6 flex flex-col justify-center select-none">
            <div className="space-y-6">
              {isValidValue(item.username) && (
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    title="复制账号"
                  >
                    <i className="fa-solid fa-copy text-gray-600 text-sm"></i>
                  </button>
                </div>
              )}
              ...
              {!isValidValue(item.username) && !isValidValue(item.password) && (
                <div className="text-center">
                  ...
                  <p className="text-base text-gray-500 font-medium">暂无登录信息</p>
                  <p className="text-sm text-gray-400 mt-2">该网站无需账号密码</p>
                </div>
              )}
            </div>
          </div>
